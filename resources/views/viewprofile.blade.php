<x-app-layout>
    <section class="min-h-screen overflow-hidden bg-gray-800">
        <div class="border m-6 rounded-sm">
            <div class="border rounded-md m-1">
                <div class="p-6 bg-gray-900 border border-green-900 rounded-lg block md:flex">
                    <div class="flex justify-center">
                        <img class="h-[350px] w-[350px] object-cover rounded-full" src="{{ asset('storage/' . $user->image ) }}" id="preview-image" alt="Post Photo">  
                    </div>
                    <div class="block md:mx-8 mt-4">
                        <span class="flex justify-center md:justify-start text-white text-4xl font-bold">{{ $user->name }}</span>
                        <span class="flex justify-center md:justify-start text-gray-400 text-xl font-semibold mt-6">Intro: </span>
                        <span class="flex justify-center md:justify-start text-gray-400 text-sm font-light tracking-wide">{{ $user->biography}}</span>
                        <span class="flex justify-center py-2 text-gray-200 items-end">Social Links</span>
                        <div class="border w-auto flex">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 border w-full px-4">
                    @foreach ($user->posts as $post)
                    <div class="w-full h-auto bg-red-200 flex gap-2">
                        <div class="w-2/5 md:w-1/3 bg-red-400 h-full p-2 flex justify-center items-start">
                            <img class="h-32 md:h-48 p-2" src="{{ asset('storage/' . $post->image) }}" alt="">
                        </div>
                        <div class="w-3/5 md:w-2/3 bg-red-400 h-full flex justify-start items-start ">
                            <div class="p-2">
                                ...
                                <div class="border h-full otruncate">
                                    <span class="">
                                        {{ $post->body }}
                                    </span>
                                </div>
