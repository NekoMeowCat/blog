<x-app-layout>
    <section class="min-h-screen overflow-hidden bg-gray-800">
        <div class="m-8 md:mx-14 p-3 rounded-sm">
            <div class="rounded-md m-1">
            ...
            <form method="post" action="{{ route("post.addPost") }}" enctype="multipart/form-data">
                @csrf
                <div class="block md:flex m-1 rounded-md">
                    <div class=" p-2 m-1 md:w-1/2">
                        <div class="w-full flex justify-center">
                            <img id="preview-image" class="h-full uploaded-image rounded-lg"
                                src="{{ asset("storage/images/picture.png") }}" alt="">
                        </div>
                        <input id="post_image" name="post_image" type="file" class="hidden" accept="image/*"
                            onchange="handleFileInputChange(event)" />
                        <label for="post_image"
                            class="mt-7 inline-flex items-center text-white bg-gradient-to-r from-teal-400 via-teal-500 to-teal-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-teal-300 dark:focus:ring-teal-800 shadow-lg shadow-teal-500/50 dark:shadow-lg dark:shadow-teal-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">
                            Upload Photo
                        </label>
                    </div>
