<x-app-layout>
    <section class="min-h-screen w-full   bg-gray-800">
        <div class="flex justify-center">
            <div class="border border-gray-500 rounded-lg w-96 md:w-[600px] m-6">

                <!-- post profile -->
                <div class="m-2 flex justify-start border border-gray-500 rounded-md flex-grow">
                    <div class="m-2 border rounded-full w-16">
                        <img class="h-full rounded-full object-cover w-full"
                            src="{{ asset("storage/" . Auth::user()->image) }}" alt="Bordered avatar">
                    </div>
                    <div class="w-5/6 border-1 m-1">
                        <div class="flex justify-start items-start">
                            <span class="font-bold text-white"> {{ $post->author }}</span>
                        </div>
                        <div class="flex justify-start items-start">
                            <small class="font-light font-poppins text-gray-400">
                                {{ $post->created_at->format('F d, Y \a\t H:i') }} </small>
                        </div>
                    </div>
                </div>
                <!-- post profile -->
                ...
                <!-- post content -->
                <div class="m-2 flex justify-start border border-gray-500 rounded-md">
                    <span class="text-start font-poppins text-gray-200 text-base p-3">
                        {{ $post->body }}
                    </span>
                </div>
                <!-- post content -->
                ...
                <!-- post comment submit -->
                <div class="m-2 flex border-1 rounded-md">
                    ...
                    @foreach ($comments as $comment)
                        <div class="flex items-start w-full mr-10 p-2 mb-2 bg-gray-700 rounded ">
                            <div class="m-1 border rounded-full w-14">
                                <img class="h-14 rounded-full object-cover w-full"
                                    src="{{ asset("storage/" . Auth::user()->image) }}" alt="Bordered avatar">
                            </div>
